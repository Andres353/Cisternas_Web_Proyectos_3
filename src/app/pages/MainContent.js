...
const MainContent = () => {
  ...
  useEffect(() => {
    const filteredOtbs = otbs.filter(otb => {
      ...
      const otbDate = new Date(otb.date.getFullYear(), otb.date.getMonth(), otb.date.getDate());
      const selectedDate = new Date(date.getFullYear(), date.getMonth(), date.getDate());
      return otbDate.getTime() === selectedDate.getTime();
    });
          ...
          <div className="otbs-list">
            {selectedDateOtbs.map((otb, index) => (
              <div key={index} className="otb-info">
                <h3>{otb.name} - Distrito {otb.district}</h3>
